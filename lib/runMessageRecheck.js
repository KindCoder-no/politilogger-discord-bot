    ...
    for (const check of database_file) {
        ...
        if (check.last_message != messages[messages.length - 1].id) {
            ...
            const exampleEmbed = new EmbedBuilder()
                ...
                .addFields(
                    { name: '\u200B', value: '\u200B' },
                    { name: 'Åpen kildekode', value: 'Sjekk ut koden her: https://github.com/KindCoder-no/politilogger-discord-bot' },
                )
            ...
            const newMessage = await oldMessage.reply({ embeds: [exampleEmbed] });
